...
function updateUI(){
  renderSteps();
  if(state.current<0){$('activity').textContent='—';$('expected').textContent='—';$('interaction').textContent='—';$('sequence').textContent='—';$('nextStep').textContent='Start experiment';$('stateBadge').textContent='STANDBY';$('stateBadge').className='badge neutral';$('persons').textContent='0';$('objects').textContent='0';$('latency').textContent='—';$('aiHud').textContent=state.ws?.readyState===1?'AI: READY':'AI: STANDBY';setConfidence(0);return}
  ...
  $('persons').textContent=state.lastResult?.persons??0;
...
function connectBackend(){
  ...
  log(`Connecting to Edge AI: ${API_BASE}`,'warn');
  try{
    state.ws=new WebSocket(WS_URL);
    state.ws.onopen=()=>{log('Edge AI backend connected');$('network').textContent='CONNECTED';$('systemPill').innerHTML='<i></i> EDGE AI CONNECTED';$('systemPill').className='pill good';updateUI();startInference();};
    ...
    state.ws.onerror=()=>{log(`Backend connection failed at ${WS_URL}`,'bad');$('network').textContent='OFFLINE';$('aiHud').textContent='AI: BACKEND OFFLINE';toast('Edge AI backend not reachable');};
    state.ws.onclose=()=>{state.ws=null;stopInference();$('network').textContent='OFFLINE';updateUI();};
  }catch(e){log(`WebSocket unavailable: ${e.message}`,'bad');}
}
function startInference(){stopInference();state.inferenceTimer=setInterval(sendFrame,300);}
...
function sendFrame(){
  ...
  state.ws.send(JSON.stringify({frame:c.toDataURL('image/jpeg',.72)}));
}
...
$('videoFile').onchange=async e=>{const f=e.target.files[0];if(!f)return;if(state.stream)stopCamera();if(state.videoURL)URL.revokeObjectURL(state.videoURL);state.videoURL=URL.createObjectURL(f);state.inputMode='video';const v=$('camera');v.srcObject=null;v.src=state.videoURL;v.controls=true;v.muted=true;v.playsInline=true;$('cameraPlaceholder').style.display='none';$('cameraStatus').textContent=`Loaded: ${f.name}`;log(`Video file loaded — ${f.name}`);toast('Video loaded');try{await v.play();log('Video playback started');}catch(err){log('Autoplay blocked — press Play on the video controls','warn');}connectBackend();};
...
$('camera').addEventListener('error',()=>{if(state.videoURL)log(`Video error: ${$('camera').error?.message||'browser could not decode this video'}`,'bad');});
...
updateUI();
log(`Console initialized — AI endpoint ${API_BASE}`);
